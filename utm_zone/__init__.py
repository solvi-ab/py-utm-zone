...
def epsg_from_lnglat(lng, lat):
    """
    Provided lat, lng, returns the EPSG code (integer) for the UTM zone best
    matching the coordinates.
    """
    offset = int(round((183 + lng) / 6.0))
...
def epsg(geojson):
    ...
    return epsg_from_lnglat(*firstCoord)
...
def proj(geojson):
    ...
    return "+proj=utm +zone=%d +%s +datum=WGS84 +units=m +no_defs" % (
        zone,
        "north" if firstCoord[1] >= 0 else "south",
    )
...
def getFirstCoord(geojson):
    ...
    firstCoord = (
        ...
        else recurse(geojson["geometry"]["coordinates"])
        if "geometry" in geojson
        else recurse(geojson["geometries"][0]["coordinates"])
        if "geometries" in geojson
        else recurse(geojson["features"][0]["geometry"]["coordinates"])
